...
export default function MapPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-6 py-8">
      <h1 className="mb-2 text-2xl font-bold text-slate-900">{t("publicMap.title")}</h1>
      <p className="mb-6 text-sm text-slate-600">
        {t("publicMap.subtitle")}
      </p>
          ...
          <Card key={sensor.id}>
            <div className="mb-2 flex items-start justify-between gap-2">
              ...
              >
                ...
                {sensor.online ? t("publicMap.online") : t("publicMap.offline")}
              </span>
            </div>

            <p className="mt-2 text-sm text-slate-700">
              {t("publicMap.waterLevel")}:{" "}
              {sensor.hasWaterLevelData ? (
                <span className="font-semibold">{sensor.lastLevelCm.toFixed(1)} cm</span>
              ) : (
                <span className="inline-flex rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-500">{t("publicMap.waitingData")}</span>
              )}
            </p>
            <p className="text-xs text-slate-500">{t("publicMap.lastUpdate")}: {formatRelativeTime(sensor.lastSeenAt ?? sensor.updatedAt, Date.now(), language)}</p>
            <p className="text-xs text-slate-500">{t("publicMap.lastUpdateWib")}: {formatTimestamp(sensor.lastSeenAt ?? sensor.updatedAt, language)}</p>
            <a
