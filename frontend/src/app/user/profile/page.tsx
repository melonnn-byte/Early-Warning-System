...
export default function ProfilePage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setMessage({ type: "error", text: t("userProfile.avatarTooLarge") });
      return;
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAvatarFile(null);
      setMessage({ type: "success", text: t("userProfile.saveSuccess") });

    } catch (err) {
      const errorMsg = (err as { response?: { data?: { message?: string } } }).response?.data?.message || t("userProfile.saveFailed");
      ...
      <main className="mx-auto max-w-3xl px-6">
        ...
        <header className="mb-8">
          <button onClick={() => router.back()} className="mb-4 text-sm font-semibold text-blue-600 hover:text-blue-800">
            &larr; {t("userProfile.back")}
          </button>
          <h1 className="text-3xl font-extrabold text-slate-900">{t("userProfile.title")}</h1>
          <p className="mt-1 text-slate-500">{t("userProfile.subtitle")}</p>
        </header>
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col items-center gap-6 sm:flex-row">
              ...
              <div className="flex flex-col items-center sm:items-start">
                <h3 className="font-bold text-slate-900">{t("userProfile.avatarTitle")}</h3>
                <p className="mb-3 text-center text-xs text-slate-500 sm:text-left">
                  {t("userProfile.avatarSubtitle")}
                </p>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-slate-700">{t("userProfile.fieldName")}</label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-slate-700">{t("userProfile.fieldEmail")}</label>
                <input
                  type="email"
                  value={user?.email || ""}
                  disabled
                ...
                />
                <p className="text-xs text-slate-400">{t("userProfile.fieldEmailDesc")}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end pt-4">
              ...
              >
                {isSubmitting ? t("userProfile.saving") : t("userProfile.saveBtn")}
              </button>
            </div>
        ...
        <ConfirmDialog
          ...
          description={t("userProfile.confirmDesc")}
          confirmText={t("userProfile.confirmYes")}
          cancelText={t("userProfile.confirmCancel")}
